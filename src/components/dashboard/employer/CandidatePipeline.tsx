...
import { Loader2, X, Mail, MapPin, ShieldCheck } from "lucide-react";
...
import { ApplicationWithDetails } from "@/lib/employer-applications";

export default function CandidatePipeline({ initialApplications }: { initialApplications: ApplicationWithDetails[] }) {
    ...
    const templateMessage = (app: ApplicationWithDetails) => {
    ...
    const openMailDraft = (app: ApplicationWithDetails) => {
    ...
    const openWhatsApp = (app: ApplicationWithDetails) => {
    ...
    const callPhoneNumber = (app: ApplicationWithDetails) => {
        const phone = app?.user?.jobSeeker?.phone;
        if (!phone) return;
        window.open(`tel:${phone}`, "_self");
    };

    const handleReportCandidate = async (app: ApplicationWithDetails) => {
        try {
            ...
            // non-blocking UX but log for debugging
            console.error("[CandidatePipeline] Failed to report candidate:", err);
        }
    };
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-start">
                ...
                <div className="flex flex-col gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        <Badge label={applications.filter((a) => a.status === "PENDING").length.toString()} variant="secondary" />
                    </div>
                    {applications
                ...
                <div className="flex flex-col gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        <Badge label={applications.filter((a) => a.status === "SHORTLISTED").length.toString()} variant="green" />
                    </div>
                    {applications
                ...
                <div className="flex flex-col gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        <Badge label={applications.filter((a) => a.status === "INTERVIEWING").length.toString()} variant="blue" />
                    </div>
                    {applications
                ...
                <div className="flex flex-col gap-4 opacity-75">
                    <div className="flex items-center gap-2">
                        ...
                        <Badge label={applications.filter((a) => a.status === "REJECTED").length.toString()} variant="red" />
                    </div>
                    {applications
                        ...
                        >
                            ...
                            <div className="mt-6 space-y-6">
                                <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    {selectedApp.screeningScore !== undefined && (
                                        <Badge 
                                            label={`${selectedApp.screeningScore}% Match`} 
                                            variant={selectedApp.screeningScore >= 80 ? "green" : selectedApp.screeningScore >= 50 ? "blue" : "yellow"} 
                                        />
                                    )}
                                ...
                                {selectedApp.screeningSummary && (
                                    <div className="rounded-2xl border border-stone-200 bg-white/80 p-4 dark:border-slate-800 dark:bg-slate-900">
                                        <div className="flex items-center gap-2 mb-2">
                                            <ShieldCheck size={16} className="text-blue-500" />
                                            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Smart Match Justification</p>
                                        </div>
                                ...
                                {(selectedApp.screeningBreakdown?.length ?? 0) > 0 && (
                                    <div>
                                        <p className="mb-2 text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Screening checklist</p>
                                ...
                                {(selectedApp.user?.jobSeeker?.skills?.length ?? 0) > 0 && (
                                    <div>
                                ...
                                {(selectedApp.user?.jobSeeker?.experience?.length ?? 0) > 0 && (
                                    <div>
