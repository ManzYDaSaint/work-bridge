...
import { X, MapPin, Briefcase, GraduationCap, Bookmark, Mail, Phone, Globe } from "lucide-react";
...
function formatDateRange(start?: string | null, end?: string | null) {
    const startLabel = formatDate(start) || "Unknown";
    const endLabel = end ? (formatDate(end) || "Present") : "Present";
                        ...
                        <div className="sticky top-0 z-10 border-b border-stone-200 bg-white/95 px-6 py-5 backdrop-blur dark:border-slate-800 dark:bg-slate-950/95">
                            ...
                            <div className="mt-4 flex flex-wrap gap-2">
                                {profile?.search_intent && (
                                    <Badge label={profile.search_intent === "SEEKING_INTERNSHIP" ? "Seeking Internship" : profile.search_intent.replaceAll("_", " ")} variant="blue" />
                                )}
                                {profile?.employment_status && (
                                    <Badge label={profile.employment_status.replaceAll("_", " ")} variant="secondary" />
                                )}
                                    ...
                                    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto]">
                                        ...
                                        <div className="space-y-4 rounded-3xl border border-stone-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900/70">
                                            <div className="flex items-center justify-between gap-4">
                                                ...
                                                >
                                                    {isSaved ? <Bookmark size={14} /> : <Bookmark size={14} />}
                                                    {isSaved ? "Saved" : "Save"}
                                                </button>
                                            </div>
