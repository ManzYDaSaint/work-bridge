...
export default function AdminAuditClient({ 
    ...
    initialMeta: { total: number; limit: number; offset: number };
    initialSearchParams: {
        offset?: string;
        userId?: string;
        action?: string;
    ...
    const offset = parseInt(searchParams.get("offset") || initialSearchParams.offset || "0");
    const userId = searchParams.get("userId") || initialSearchParams.userId || "";
    const action = searchParams.get("action") || initialSearchParams.action || "";
    // path filter intentionally unused in UI for now
        ...
        <div className="space-y-8 pb-20">
            ...
            <div className="flex flex-col xl:flex-row gap-4 items-center justify-between bg-white/50 backdrop-blur-sm p-4 rounded-[2rem] border border-slate-200 shadow-sm">
                ...
                <button
                    ...
                    className="h-11 px-8 bg-slate-900 text-white rounded-xl font-black text-[10px] uppercase tracking-widest active:scale-95 transition-all shadow-lg flex items-center gap-2 disabled:opacity-50"
                >
            ...
            <div className="glass-effect rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden bg-white">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center px-10 py-6 bg-slate-50 border-t border-slate-100 gap-4">
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Registry Total: {initialMeta.total}</span>
                    <div className="flex gap-2">
                        <button
                            disabled={offset === 0}
                            onClick={() => updateFilters({ offset: Math.max(offset - limit, 0) })}
                        ...
                        <button
                            disabled={offset + limit >= initialMeta.total}
